...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      ...
      /* --- Cadre orange : exporter les événements --- */
      .export-box {
    ...
    tags$div(
      ...
      sidebarPanel(
        ...
        selectInput("type_evenement", "Type d'événement",
                    choices = c("", "Humain", "Animal", "Environnement", "Humain et Animal", "Autre"),
                    selected = ""),
        div(
          class = "large-input",
          textAreaInput(
            inputId = "sources",
            label = HTML(
              'Sources <span class="info-icon" title="Saisir une source par ligne au format : Nom (lien). Exemple : WHO (https://www.who.int)
ECDC (https://www.ecdc.europa.eu)">i</span>'
        ...
        # --- Téléverser des graphiques
        h4("Téléverser des images"),
        tags$div(style = "margin-bottom: 10px;",
                 tags$label(HTML(
                   'Téléverser des graphiques (liés à l’événement sélectionné)
     <span class="info-icon" title="⚠️ Astuce : Veuillez d’abord sélectionner ou charger un événement dans le tableau avant de téléverser des graphiques. Les fichiers téléchargés seront automatiquement associés à cet événement et apparaîtront ensuite dans le rapport Word.">i</span>'
  ...
  "))
        ...
        # ---  "Mettre à jour" + "Enregistrer comme nouvel événement" ---
        div(style = "margin-top: 20px;",
      ...
      mainPanel(
          ...
          column(
            ...
            selectInput(
              ...
              choices = c(
                ...
                "Mesures"                   = "Mesures",
                "Résumé"                    = "Resume",
                "Évaluation des risques"    = "Evaluation_Risque",
        ...
        div(class = "table-actions",
            ...
            actionButton("clear_all",  "Désélectionner tous les événements", icon = icon("ban"),
                         class = "btn btn-outline-secondary btn-sm"),
            actionButton("zoom_table", "Zoom tableau", icon = icon("expand"),
                         class = "btn btn-outline-secondary btn-sm")
        ),
